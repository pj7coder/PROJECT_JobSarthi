...
(function () {
  ...
  window.JobDrawer = {
    ...
    open(job, opts) {
      ...
      if (cb) cb.checked = savedIds.includes(job.id);
    ...
    _onSaveChange(checked) {
      ...
      if (checked) {
        if (!savedIds.includes(_currentJob.id)) savedIds.push(_currentJob.id);
      } else {
        savedIds = savedIds.filter(id => id !== _currentJob.id);
      }
