...
app.get("/api/resume/view", async (req, res) => {
  try {
    ...
    const isCloudinaryPdfError = err.message.includes("Cloudinary Security settings") || err.message.includes("401") || err.message.includes("403");
    
    let errorDetailHtml = "";
    if (isCloudinaryPdfError) {
      errorDetailHtml = `
        <div class="header">
          <div class="error-icon-wrapper">
            <span class="error-icon">🔒</span>
          </div>
          <h1>Cloudinary PDF Restriction Active</h1>
          <p class="subtitle">Cloudinary blocked the delivery of this resume because PDF delivery is restricted under your account settings.</p>
        </div>

        <div class="steps-container">
          <div class="steps-title">
            <span>⚙️</span> How to Allow PDF Delivery
          </div>
          <ul class="steps-list">
            <li class="step-item">
              <div class="step-number">1</div>
              <div class="step-text">Log in to your <strong>Cloudinary Console</strong>.</div>
            </li>
            <li class="step-item">
              <div class="step-number">2</div>
              <div class="step-text">Click the <strong>Settings gear icon</strong> in the bottom-left corner of the sidebar.</div>
            </li>
            <li class="step-item">
              <div class="step-number">3</div>
              <div class="step-text">Navigate to the <strong>Security</strong> tab.</div>
            </li>
            <li class="step-item">
              <div class="step-number">4</div>
              <div class="step-text">Locate the checkbox for <strong>"Restrict PDF and ZIP files delivery"</strong> and <strong>uncheck</strong> it.</div>
            </li>
            <li class="step-item">
              <div class="step-number">5</div>
              <div class="step-text">Scroll down and click <strong>Save</strong>. You might need to refresh this tab to reload the resume.</div>
            </li>
          </ul>
        </div>

        <div class="alternative-box">
          <strong>💡 Alternative:</strong> You can also choose to re-upload this applicant's resume. Any newly uploaded resumes are saved automatically as unrestricted <strong>raw assets</strong>, which will bypass this security block without requiring settings changes.
        </div>
      `;
    } else {
      errorDetailHtml = `
        <div class="header">
          ...
          <p class="subtitle">We encountered an unexpected error while loading this file.</p>
        </div>

        <div class="steps-container" style="background: rgba(239, 68, 68, 0.05); border-color: rgba(239, 68, 68, 0.2);">
          <div class="steps-title" style="color: var(--accent);">
            <span>Details:</span>
          </div>
          <div style="font-family: monospace; font-size: 0.9rem; color: var(--text-muted); word-break: break-all; line-height: 1.6;">
            ${err.message}
          </div>
        </div>
      `;
    ...
    res.status(500).setHeader("Content-Type", "text/html").send(`
      ...
      <head>
        ...
        <title>File Display Error - JobSarthi</title>
